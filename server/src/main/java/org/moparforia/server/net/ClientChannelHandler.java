...
public class ClientChannelHandler extends ChannelDuplexHandler {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
        ...
        ctx.channel().attr(ClientState.CLIENT_STATE_ATTRIBUTE_KEY).get().setLastActivity(System.currentTimeMillis());
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) {
        if (evt instanceof IdleStateEvent e) {
            if (e.state() == IdleState.READER_IDLE) {
                ...
                long time = System.currentTimeMillis() - state.getLastActivity();
                if (time > 20000) {
                    ctx.channel().close();
                } else if (time > 5000) {
