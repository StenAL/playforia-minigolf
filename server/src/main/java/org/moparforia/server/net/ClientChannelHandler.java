...
public class ClientChannelHandler extends ChannelDuplexHandler {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
        ...
        ctx.channel()
                .attr(ClientState.CLIENT_STATE_ATTRIBUTE_KEY)
                .get()
    ...
    public void channelInactive(ChannelHandlerContext ctx) {
        ...
        if (player != null && server.hasPlayer(player.getId())) {
            int id = player.getId();
            server.addEvent(
                    new TimedEvent(30_000) { // todo: confirm this time
                        @Override
                        public void process(Server server) {
                            if (server.hasPlayer(id)
                                    && !server.getPlayer(id).getChannel().isOpen()) {
